                ...
                <div class="p-4 pb-0 mb-0 bg-white dark:bg-slate-800 border-b-0 rounded-t-2xl">
                    <div class="flex flex-wrap -mx-3">
                        <div class="flex items-center w-full max-w-full px-3 md:w-5/12">
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-300 ml-2 hidden md:block">Suivi des patients en attente et en consultation</p>
                        </div>
                        ...
                        <div class="w-full md:w-3/12 px-3 mb-4 md:mb-0">
                            <form method="GET" action="{{ route('queues.index') }}" class="relative">
                                ...
                                <input type="text" 
                                       name="search" 
                                       value="{{ $search ?? '' }}" 
                                       class="w-full pl-10 pr-4 py-2 text-sm rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                       placeholder="Rechercher un médecin...">
                                <div class="absolute left-3 top-2.5 text-gray-400 dark:text-slate-400">
                        ...
                        <div class="flex items-center justify-end w-full md:w-4/12 px-3">
                            <form method="GET" action="{{ route('queues.index') }}" class="flex items-center mr-4">
                                ...
                                <input type="date" 
                                       name="date" 
                                       class="text-sm rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-100 p-2 mr-2" 
                                       value="{{ $date->format('Y-m-d') }}"
                                       onchange="this.form.submit()"
                                       style="width: 150px;">
                                <a href="{{ route('queues.index') }}" class="p-2 text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white" title="Réinitialiser">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                    </svg>
                                </a>
                            </form>
                            <span class="px-3 py-1 text-sm font-medium text-gray-700 dark:text-slate-200 bg-gray-100 dark:bg-slate-900 rounded-full whitespace-nowrap">
                ...
                <div class="flex-auto p-4">
                                        ...
                                        <tr class="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-10 w-10 bg-gray-200 dark:bg-slate-700 rounded-full flex items-center justify-center">
                                                        <svg class="h-6 w-6 text-gray-500 dark:text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-900 dark:text-slate-100 font-medium">{{ $doctor->doctorProfile->specialty->name ?? 'Non spécifiée' }}</div>
                                                <div class="text-sm text-gray-500 dark:text-slate-300">{{ $doctor->doctorProfile->specialty->department->name ?? '' }}</div>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <span class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $doctor->pending_count > 0 ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/60 dark:text-blue-200' : 'bg-gray-100 text-gray-800 dark:bg-slate-700 dark:text-slate-200' }}">
                                                    {{ $doctor->pending_count }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                @if($doctor->doctorAppointments->isNotEmpty())
                                                    <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900/60 dark:text-green-200">
                                                        {{ $doctor->doctorAppointments->first()->patient->first_name }}
                                                        {{ $doctor->doctorAppointments->first()->patient->last_name }}
                                                    </span>
                                                @else
                                                    <span class="text-sm text-gray-500 dark:text-slate-400">Aucun</span>
                                                @endif
                                            </td>
                    ...
                    @else
                        <div class="p-6 text-center">
                            <p class="text-gray-500 dark:text-slate-300">Aucun médecin trouvé pour le moment.</p>
                        </div>
                    @endif
                </div>
            </div>
